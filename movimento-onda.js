...
function setup(){
    ...
    meuSlider = createSlider(1, 400, 1, 1);
    meuSlider.position(width*posicaoX, 20);

    numeroDePontos = createSlider(1, 10**7, 650, 1);
...
function draw(){
    ...
    while(listaCirculos.length < numeroDeCirculos.value()){
        ...
        listaCirculos.push(criarCirculoFourier(tamanho));
    }
    while(listaCirculos.length > numeroDeCirculos.value()){
        listaCirculos.pop();
    }
    ...
    for(let i=0; i<listaCirculos.length; i++){
        ...
        listaPontos.unshift({x: novoX, y: novoY, posicao: i});
    }
        ...
        if(checkBoxes[j].checked()){
            ...
            if(mostrarPoligono){
                ...
                for(let i=0; i<listaPontos.length; i++){
                    const posicaoX = parseInt(i/listaCirculos.length);

                    if(listaPontos[i].posicao == j){
                        vertex(posicaoX, listaPontos[i].y);
            ...
            //Exibe pontilhado
                ...
                for(let i=0; i<listaPontos.length; i++){
                    ...
                    if(listaPontos[i].posicao == j){
                        point(posicaoX, listaPontos[i].y);
                    }                  
                }
            }
        }
...
function criarCirculoFourier(numero){
    const tamanhoInicial = 1000;
    return {r: tamanhoInicial/(Math.PI* (numero*2-1)), v: (1)};
}

function criarCirculoIndependente(tamanho){
    return {r:tamanhoCirculo(tamanho), v:10.0/tamanhoCirculo(tamanho)};
}
